...
export default function SnackDetailsScreen() {
  ...
  // Check if snack is bookmarked by current user
  useEffect(() => {
    if (!user || !id || id.startsWith("sp-")) return;
    supabase
      .from("bookmarks")
      .select("id")
      .eq("user_id", user.id)
  ...
  const toggleBookmark = async () => {
    ...
    if (id.startsWith("sp-")) return;

    if (isBookmarked) {
      setIsBookmarked(false);
      await supabase.from("bookmarks").delete().eq("user_id", user.id).eq("snack_id", id);
    } else {
      setIsBookmarked(true);
      await supabase.from("bookmarks").insert({ user_id: user.id, snack_id: id });
    }
  };
  ...
  return (
    <Screen>
        ...
        <View style={styles.headerRightControls}>
          ...
          <Pressable onPress={toggleBookmark} style={styles.bookmarkButton}>
            <Ionicons
              ...
              size={34}
              color={isBookmarked ? colors.primary : colors.ink}
            />
          </Pressable>
        </View>
      </View>
      ...
      <View style={styles.actions}>
        {!id.startsWith("sp-") ? <Button onPress={rate} icon="star">Rate it</Button> : null}
      </View>
...
const styles = StyleSheet.create({
  ...
  titleRow: { flexDirection: "row", alignItems: "flex-start", gap: spacing.md, marginTop: spacing.md },
  headerRightControls: { alignItems: "flex-end", gap: spacing.xs },
  bookmarkButton: {
